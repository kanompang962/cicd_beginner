...
This project demonstrates a full CI/CD pipeline for deploying a web application consisting of an Angular frontend and a .NET 8 backend, containerized using Docker, and deployed to a VPS with Nginx as a reverse proxy. The application is accessible at tainchaidev.online.
Project Overview
...
Remove /etc/nginx/sites-enabled/default and restart Nginx:sudo rm /etc/nginx/sites-enabled/default
...
Check container logs:docker logs dotnet-api


Test endpoints directly:curl -I http://localhost:5208
...
Verify DNS resolution for tainchaidev.online:ping tainchaidev.online
...
HTTPS: Configure Let's Encrypt for SSL:sudo apt install certbot python3-certbot-nginx
